...
const ChatRecord = () => {
  ...
  const fetchData = async () => {
    try {
      const response1 = await axios.get("http://localhost:8081/admin/kochat");
      const response2 = await axios.get("http://localhost:8081/admin/enchat")
  ...
  const WrongAnswerK = "죄송합니다. 이해하지 못했습니다. 키워드로 질문하거나 좀 더 정확하게 말씀해주세요. 예 : 제품 등록"
  const WrongAnswerE = "I'm sorry. I didn't understand. Please ask me a ke…e more accurately. Example : Product Registration"
  ...
  const CountWrongAnswer = () => {
    const Answer = countAnswersWithSpecificValue(Kodata, WrongAnswerK)+countAnswersWithSpecificValue(Endata, WrongAnswerE);
    return Answer;
  }
  ...
  const allKeys = [...new Set([...Object.keys(Kodata), ...Object.keys(Endata)])];
                  ...
                  <ul className="flex">
                    <li>
                      <button className="flex w-24 cursor-default flex-col items-center rounded-l-md border border-r-0 bg-white p-2 focus:bg-yellow-200 focus:outline-none">
                        <p className="text-lg font-semibold">{countAllMap()-CountWrongAnswer()}</p>
                    ...
                    <li>
                      <button className="flex w-24 cursor-default flex-col items-center rounded-r-md border bg-white p-2 focus:bg-yellow-200 focus:outline-none">
                        <p className="text-lg font-semibold">{CountWrongAnswer()}</p>
              ...
              <table className="flex h-full min-h-0 w-full flex-col border-t shadow-md">
                ...
                <tbody className="flex min-h-0 w-full flex-1 flex-col px-4">
                  ...
                  {sortedChats.map((value, index) => (
                    <tr key={index} role="row" className="flex cursor-pointer border-b hover:bg-blue-100" style={{ height: 'auto' }} >
                      ...
                      <td className="flex w-24 justify-center truncate px-1 py-3">
                        {value.answer !== WrongAnswerK && value.answer !== WrongAnswerE ? 'Y' : 'N'}
                      </td>
                    </tr>
                  ))}
